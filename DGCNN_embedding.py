...
class DGCNN(nn.Module):
    def __init__(self, output_dim, num_node_feats, num_edge_feats, latent_dim=[32, 32, 32, 1], k=30, conv1d_channels=[16, 32], conv1d_kws=[0, 5]):
        ...
        conv1d_kws[0] = self.total_latent_dim*3
	...
	for g_idx in range(n_graphs):
	    A = n2n_sp2[start_idx:start_idx+graph_sizes[g_idx], start_idx:start_idx+graph_sizes[g_idx]]
        ...
        h = self.sortpooling_embedding(node_feat, edge_feat, n2n_sp, n2n_sp2, e2n_sp, subg_sp, graph_sizes, node_degs, node_degs2)

        return h

    def sortpooling_embedding(self, node_feat, edge_feat, n2n_sp, n2n_sp2, e2n_sp, subg_sp, graph_sizes, node_degs, node_degs2):
	    ...
	    """For 2-hop"""
            n2npool2 = gnn_spmm(n2n_sp2, cur_message_layer2) + cur_message_layer2  # Y = (A + I) * X
            node_linear2 = self.conv_params[lv](n2npool2)  # Y = Y * W
            normalized_linear2 = node_linear.div(node_degs2)  # Y = D^-1 * Y
            cur_message_layer2 = F.tanh(normalized_linear2)
            cat_message_layers.append(cur_message_layer2)
	    """For 0-hop, (A = 0)"""
            n2npool0 = cur_message_layer0  # Y = (A + I) * X
            node_linear0 = self.conv_params[lv](n2npool0)  # Y = Y * W
            cur_message_layer0 = F.tanh(node_linear0)           
            cat_message_layers.append(cur_message_layer0)
            lv += 1
        ...
        batch_sortpooling_graphs = torch.zeros(len(graph_sizes), self.k, self.total_latent_dim*3)
	#print('Total latent dim: ', self.total_latent_dim )
        ...
        for i in range(subg_sp.size()[0]):
            ...
            if k < self.k:
                to_pad = torch.zeros(self.k-k, self.total_latent_dim*3)
                if isinstance(node_feat.data, torch.cuda.FloatTensor):
                    to_pad = to_pad.cuda()

                to_pad = Variable(to_pad)
                sortpooling_graph = torch.cat((sortpooling_graph, to_pad), 0)
	    #print('Sort pooling graph size: ', sortpooling_graph.shape)
            batch_sortpooling_graphs[i] = sortpooling_graph
            accum_count += graph_sizes[i]

        ''' traditional 1d convlution and dense layers '''
        to_conv1d = batch_sortpooling_graphs.view((-1, 1, self.k * self.total_latent_dim*3))
