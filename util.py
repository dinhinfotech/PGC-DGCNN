...
cmd_opt.add_argument('-result_file', default=None, help='the result file')
cmd_opt.add_argument('-result_paras', default=None, help='saving parameters values')
cmd_opt.add_argument('-patience', default=10, type=int, help='patience value')
...
def load_data():
    ...
    if cmd_args.test_number == 0:
        ...
        train_idxes_ori = np.loadtxt('data/%s/10fold_idx/train_idx-%d.txt' % (cmd_args.data, cmd_args.fold), dtype=np.int32).tolist()
	t = int(round(len(train_idxes_ori)*0.875))
	train_idxes = train_idxes_ori[:t]
	validation_idxes = train_idxes_ori[t:]
